...
static BiasedLocking::Condition revoke_bias(oop obj, bool allow_rebias, bool is_bulk, JavaThread *requesting_thread) {
    ...
    if (!is_bulk) {
        ResourceMark rm;
        log_info(biasedlocking)("Revoking bias of object " INTPTR_FORMAT
        ", mark "
        INTPTR_FORMAT
        ", type %s, prototype header " INTPTR_FORMAT
        ...
        ResourceMark rm;
        log_trace(biasedlocking)("Revoking bias of object " INTPTR_FORMAT
        " , mark "
        INTPTR_FORMAT
        " , type %s , prototype header " INTPTR_FORMAT
        " , allow rebias %d , requesting thread " INTPTR_FORMAT,
    ...
    for (int i = 0; i < cached_monitor_info->length(); i++) {
        ...
        if (mon_info->owner() == obj) {
            log_trace(biasedlocking)("   mon_info->owner (" PTR_FORMAT
            ") == obj (" PTR_FORMAT
            ")",
...
static BiasedLocking::Condition bulk_revoke_or_rebias_at_safepoint(oop o,
    ...
    if (bulk_rebias) {
            ...
            for (JavaThread *thr = Threads::first(); thr != NULL; thr = thr->next()) {
                ...
                for (int i = 0; i < cached_monitor_info->length(); i++) {
                    ...
                    if ((owner->klass() == k_o) && mark->has_bias_pattern()) {
                        ...
                        assert(mark->bias_epoch() == prev_epoch || mark->bias_epoch() == cur_epoch,
                               "error in bias epoch adjustment");
        ...
        revoke_bias(o, attempt_rebias_of_object && klass->prototype_header()->has_bias_pattern(), true,
                    requesting_thread);
    } else {
        if (log_is_enabled(Info, biasedlocking)) {
            ResourceMark rm;
...
public:
    VM_RevokeBias(Handle *obj, JavaThread *requesting_thread)
            : _obj(obj), _objs(NULL), _requesting_thread(requesting_thread), _status_code(BiasedLocking::NOT_BIASED) {}

    VM_RevokeBias(GrowableArray<Handle> *objs, JavaThread *requesting_thread)
            : _obj(NULL), _objs(objs), _requesting_thread(requesting_thread), _status_code(BiasedLocking::NOT_BIASED) {}
...
public:
    ...
    virtual void doit() {
        _status_code = bulk_revoke_or_rebias_at_safepoint((*_obj)(), _bulk_rebias, _attempt_rebias_of_object,
                                                          _requesting_thread);
        clean_up_cached_monitor_info();
    }
};
    ...
    if (mark->is_biased_anonymously() && !attempt_rebias) {
        ...
        if (!prototype_header->has_bias_pattern()) {
            ...
            if (attempt_rebias) {
                ...
                markOop biased_value = mark;
                markOop rebiased_prototype = markOopDesc::encode((JavaThread *) THREAD, mark->age(),
...
int *
BiasedLocking::anonymously_biased_lock_entry_count_addr() { return _counters.anonymously_biased_lock_entry_count_addr(); }
